...
function ProductCarousel({ collections }) {
  ...
  const onDragEnd = (x) => {
    ...
    if (Math.abs(diff) > 50) {
      clearInterval(timerRef.current);
      goTo(index + (diff > 0 ? 1 : -1), slides.length);
    }
    ...
    <section className="py-10 sm:py-14 bg-navy-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-6 sm:mb-10">
          <span className="text-champagne-400 font-bold text-xs sm:text-sm uppercase tracking-widest">
            Just In
          </span>
          <h2 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-white mt-1">
            New Arrivals
          </h2>
        ...
        >
          {/* Slide image */}
          <div
            className="relative h-52 xs:h-64 sm:h-80 md:h-96 lg:h-[440px] bg-navy-800 cursor-pointer"
          ...
          {/* Info bar */}
          <div
            className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 lg:p-8 cursor-pointer"
            onClick={() => navigate(`/product/${product._id}`)}
          >
            <p className="text-champagne-300 text-[10px] sm:text-xs font-mono mb-1">
              {product.itemCode}
            </p>
            <h3 className="font-display font-bold text-white text-lg sm:text-2xl lg:text-3xl leading-tight mb-2 line-clamp-1">
              {product.name}
            </h3>
          ...
          {slides.length > 1 && (
            <>
              <button
                ...
                className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/15 hover:bg-white/30 backdrop-blur-sm text-white rounded-full flex items-center justify-center transition-all z-10"
              >
                <svg
              ...
              <button
                onClick={(e) => handleNav(1, e)}
                className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 bg-white/15 hover:bg-white/30 backdrop-blur-sm text-white rounded-full flex items-center justify-center transition-all z-10"
              >
                <svg
                  className="w-4 h-4 sm:w-5 sm:h-5"
                  fill="none"
          ...
          <div className="flex items-center justify-center gap-2 mt-4">
            {slides.map((_, i) => (
              <button
                ...
                onClick={() => {
                  clearInterval(timerRef.current);
                  goTo(i, slides.length);
                }}
                className={`rounded-full transition-all duration-300 ${
                  i === index
                    ? "w-6 sm:w-8 h-2 bg-champagne-400"
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 text-center">
          <div className="animate-float mb-6 sm:mb-8">
            <img
              ...
              className="h-24 xs:h-28 sm:h-36 md:h-44 mx-auto drop-shadow-2xl"
            />
          </div>
          <div className="page-enter">
            ...
            <div className="flex flex-col xs:flex-row gap-3 sm:gap-4 justify-center px-4 xs:px-0">
              <a
                href="#products"
                className="inline-flex items-center justify-center gap-2 bg-champagne-500 hover:bg-champagne-600 text-white font-bold px-6 sm:px-8 py-3 sm:py-4 rounded-2xl transition-all duration-300 hover:-translate-y-1 hover:shadow-xl text-sm sm:text-lg"
              >
                Shop Now
                <svg
                  className="w-4 h-4 sm:w-5 sm:h-5"
              ...
              <a
                href="#collections"
                className="inline-flex items-center justify-center gap-2 border-2 border-white/30 text-white hover:bg-white/10 font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-2xl transition-all duration-300 text-sm sm:text-lg backdrop-blur-sm"
              >
                View Collections
              </a>
            </div>
      ...
      <section id="collections" className="py-12 sm:py-16 lg:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8 sm:mb-12">
            ...
            <h2 className="section-title text-navy-800 mt-2">
              Our Collections
            </h2>
          </div>
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
              ...
              {collections.map((col) => (
                <CollectionCard
                  ...
                  onClick={() => handleCollection(col._id)}
                />
              ))}
            </div>
          ) : (
            <p className="text-center text-gray-400 py-8 text-sm">
              Add collections from the admin panel to display here.
            </p>
          )}
        </div>
      </section>
            ...
            {[
              {
                ...
                icon: "🔍",
                title: "Browse & Select",
                desc: "Explore collections, search for products, and pick what you need.",
              },
              {
                step: "02",
                icon: "🛒",
                title: "Add to Cart",
                desc: "Choose your quantities and add products to your cart.",
              },
              {
                step: "03",
                icon: "📱",
                title: "Order via WhatsApp",
                desc: "Fill your details and we'll send an invoice straight to WhatsApp.",
              },
            ].map((s) => (
              <div
                key={s.step}
                className="text-center bg-white rounded-2xl p-6 sm:p-8 shadow-sm hover:shadow-lg transition-shadow"
              >
                <div className="text-4xl sm:text-5xl mb-3 sm:mb-4">
                  {s.icon}
                </div>
                <div className="font-mono text-xs sm:text-sm text-champagne-500 font-bold mb-2">
                  STEP {s.step}
                </div>
                <h3 className="font-display font-bold text-lg sm:text-xl text-navy-800 mb-2 sm:mb-3">
                  {s.title}
                </h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
            ...
            <div className="relative w-full sm:w-80 lg:w-96">
              ...
              <input
                ...
                className="input pl-9 sm:pl-12 pr-8 sm:pr-10"
              />
              {search && (
                <button
                  onClick={() => setSearch("")}
          ...
          {collections.length > 0 && (
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-3 mb-6 sm:mb-8">
              {[{ _id: "all", name: "All Products" }, ...collections].map(
                (col) => (
                  <button
                    key={col._id}
                    onClick={() => handleCollection(col._id)}
          ...
          {loading ? (
            ...
            <div className="text-center py-16 sm:py-24">
              ...
              <h3 className="font-display font-bold text-xl sm:text-2xl text-gray-500 mb-2">
                No products found
              </h3>
                ...
                return (
                  <button
                    ...
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl font-semibold text-xs sm:text-sm transition-all ${
                      currentPage === page
                        ? "bg-navy-600 text-white shadow-md"
                        : "border border-gray-200 hover:bg-navy-50 text-gray-600"
                    }`}
                  >
                    {page}
                  </button>
...
function CollectionCard({ name, image, active, onClick }) {
  const gradients = [
    ...
    "from-indigo-700 to-navy-900",
    "from-teal-700 to-navy-800",
    "from-navy-600 to-slate-800",
  ];
  const hash = name
    .split("")
  ...
  return (
    <button
      ...
      className={`group relative rounded-xl sm:rounded-2xl overflow-hidden aspect-square cursor-pointer transition-all duration-300 ${
        active
          ? "ring-4 ring-champagne-500 shadow-xl scale-105"
    ...
    >
      {image ? (
        ...
        <div
          className={`w-full h-full bg-gradient-to-br ${gradient} flex flex-col items-center justify-center gap-1`}
        >
          <span className="font-display font-black text-white/20 text-4xl sm:text-5xl leading-none select-none">
            {initials}
          </span>
        </div>
      )}
      <div
        className={`absolute inset-0 flex items-end p-2 sm:p-3 ${
          image
            ? "bg-gradient-to-t from-black/70 to-transparent"
            : "bg-gradient-to-t from-black/40 to-transparent"
        }`}
      >
        <span className="font-display font-bold text-xs sm:text-sm leading-tight text-white drop-shadow">
          {name}
        </span>
      </div>
      {active && (
