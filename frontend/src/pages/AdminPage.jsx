...
className={setupErrors[f.key] ? 'border-red-400' : ''}
placeholder={f.ph}
autoComplete="new-password"
/>
) : (
...
useEffect(() => {
  api.get('/products?limit=500').then((r) => setAllProducts(r.data.products || [])).catch(() => {})
}, [])

// Compute suggestions from allProducts as user types
useEffect(() => {
            ...
            {suggestions.map((p) => {
              ...
              const highlight = (text) => {
                ...
                return (
                  <>
                    {text.slice(0, idx)}
                    <span className="bg-champagne-100 text-champagne-700 font-bold">{text.slice(idx, idx + q.length)}</span>
              ...
              return (
                ...
                >
                  {p.images?.[0] ? (
                    <img src={p.images[0]} alt="" className="w-8 h-8 rounded-lg object-cover flex-shrink-0 border border-gray-100" />
                  ) : (
                    <div className="w-8 h-8 rounded-lg bg-gray-100 flex items-center justify-center flex-shrink-0 text-sm">📦</div>
                  )}
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-medium text-gray-800 truncate">{highlight(p.name)}</p>
                    <p className="text-xs text-gray-400 font-mono">{highlight(p.itemCode)}</p>
                  </div>
                  <span className="text-xs text-navy-600 font-semibold flex-shrink-0">₹{p.salesPrice}</span>
                </button>
              )
            })}
          </div>
...
const PasswordInput = ({ value, onChange, className = '', placeholder, autoComplete, ...rest }) => {
      ...
      {[
        ...
        <div key={key}>
          ...
          <PasswordInput
            ...
            className={pwErrors[key] ? 'border-red-400' : ''}
            placeholder={ph}
